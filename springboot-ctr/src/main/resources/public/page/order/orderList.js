layui.use(['form','layer','table','laytpl','laydate','util'],function(){
    ...
    var tableIns = table.render({
        ...
        cols : [[
            ...
            {field: 'orderNo', title: '订单号', minWidth:100, align:"center"},
            // {field: 'userId', title: '订单支付者', align:'center'},
            // {field: 'shippingId', title: '地址号', align:'center'},
            {field: 'payment', title: '支付金额', align:'center'},
            {field: 'status', title: '支付状态(10:未支付|20:已支付|40发货)', align:'center'},
            {field: 'createTime',templet: '<div>{{ layui.util.toDateString(d.createTime, "yyyy-MM-dd HH:mm:ss") }}</div>',  title: '创建日期', align:'center'},
    ...
    //                 body.find("#email").val(edit.email);  //性别
    ...
    table.on('tool(orderList)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        if(layEvent === 'edit'){ //编辑
            layer.confirm('确定发货？',{icon:3, title:'提示信息'},function(index){
                $.post("/api/v1/admin/updateOrderStatus",{orderId:data.id},function (resposne) {
                    if(resposne.code===1){
                        layer.msg("发货成功");
                        tableIns.reload();
                        layer.close(index);
            ...
            layer.confirm('确定删除此用户？',{icon:3, title:'提示信息'},function(index){
                $.post("/api/v1/admin/deleteOrder",{orderId:data.id},function (resposne) {
                    if(resposne.code===1){
                        layer.msg("删除成功");
                        tableIns.reload();
                        layer.close(index);
